...
__host__ __device__ void calculateAttributes(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, Surfel* surfel) {
	int i, j;
...
__device__ void surfaceSplatStep3Gpu(int width, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, Surfel* surfel, int index) {
	...
	if (q < zbf_cutoffRadius_2)
		if (zMin <= zbufitem->zMax) {
			...
			// add color contribution
			atomicAdd(&zbufitem->c[0], red * w);
			atomicAdd(&zbufitem->c[1], green * w);
			atomicAdd(&zbufitem->c[2], blue * w);

			// normals
			atomicAdd(&zbufitem->n[0], n[0] * w);
			atomicAdd(&zbufitem->n[1], n[1] * w);
			atomicAdd(&zbufitem->n[2], n[2] * w);
		}
}

__global__ void calculateAttributesKernel(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, int numSurfels, Surfel* surfels) {
	int i;
...
void projectGpu(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, int numSurfels, Surfel* surfels) {
	...
	calculateAttributesKernel<<<512, 512>>>(width, height, warper, zBufferProperty, zBuffer, filterLUT, numSurfels, surfels);
