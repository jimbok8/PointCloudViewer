...
__host__ __device__ void calculateAttributes(int width, int height, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, float x0, float y0, float z, float n[3], Surfel* surfel, int l, float scale_stoo, float scale_otoc, float vp_sx, float vp_sy, float vp_tx, float vp_ty) {
	...
	e = f < -threshold_c3 ? -threshold_c3 : f;
...
__host__ __device__ void projectSample(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, Surfel* surfel) {
	int i, j;
	...
	if (z_c > wrp_frustum_nearplane && z_c < wrp_frustum_farplane) {
		...
		if ((xImg >= -xPad) && (xImg < width + xPad) && (yImg >= -yPad) && (yImg < height + yPad)) {
			...
			calculateAttributes(width, height, zBufferProperty, zBuffer, filterLUT, xImg, yImg, z_c, n, surfel, 0, stoo_scaling, otoc_scaling, vp_sx, vp_sy, vp_tx[0], vp_ty[0]);
		}
		else {
			surfel->xMin = 0;
			surfel->xMax = -1;
...
void surfaceSplatStep1(int width, int height, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, Surfel* surfel) {
	float zbf_cutoffRadius_2 = zBufferProperty->cutoffRadius * zBufferProperty->cutoffRadius;
		...
		for (X = xMin; X <= xMax; X++) {
			...
			if (q < zbf_cutoffRadius_2)
				...
				if (zMin <= zBuffer[i].zMax) {
					...
					zBuffer[i].c[0] += red * w;
					zBuffer[i].c[1] += green * w;
					zBuffer[i].c[2] += blue * w;
...
void project(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, int numSurfels, Surfel* surfels) {
	...
	for (int i = 0; i < numSurfels; i++)
		surfaceSplatStep1(width, height, zBufferProperty, zBuffer, filterLUT, &surfels[i]);
	for (int i = 0; i < numSurfels; i++)
...
__device__ void surfaceSplatStep1Gpu(int width, int height, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, Surfel* surfel, int index) {
	float zbf_cutoffRadius_2 = zBufferProperty->cutoffRadius * zBufferProperty->cutoffRadius;
	float _zbf_cutoffRadius_2 = 1 / zbf_cutoffRadius_2;

	int xMin, yMin, yMax;	// bounding box of the ellipse to be rasterized
	float zMin, x0, y0, a, b, c;
	int X, Y, i;
	float x, y, q;
	...
	Y = yMin + index % (yMax - yMin + 1);
	X = xMin + index / (yMax - yMin + 1);
...
__device__ void surfaceSplatStep3Gpu(int width, int height, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, Surfel* surfel, int index) {
	...
	if (q < zbf_cutoffRadius_2)
		if (zMin <= zBuffer[i].zMax) {
			...
			// add color contribution
			atomicAdd(&zBuffer[i].c[0], red * w);
			atomicAdd(&zBuffer[i].c[1], green * w);
			atomicAdd(&zBuffer[i].c[2], blue * w);
...
__global__ void projectSampleKernel(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, int numSurfels, Surfel* surfels) {
...
__global__ void surfaceSplatStep1Kernel(int width, int height, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, int numSurfels, Surfel* surfels) {
	...
	while (i < numSurfels) {
		...
		while (j < surfels[i].area) {
			surfaceSplatStep1Gpu(width, height, zBufferProperty, zBuffer, filterLUT, &surfels[i], j);
			j += blockDim.x;
		}
		i += gridDim.x;
	}
...
void projectGpu(int width, int height, Warper* warper, ZBufferProperty* zBufferProperty, ZBufferItem* zBuffer, float* filterLUT, int numSurfels, Surfel* surfels) {
	...
	projectSampleKernel<<<512, 512>>>(width, height, warperGpu, zBufferProperty, zBuffer, filterLUT, numSurfels, surfels);
	surfaceSplatStep1Kernel<<<512, 512>>>(width, height, zBufferProperty, zBuffer, filterLUT, numSurfels, surfels);
	surfaceSplatStep2Kernel<<<512, 512>>>(width, height, zBufferProperty, zBuffer, filterLUT, numSurfels, surfels);
	surfaceSplatStep3Kernel<<<512, 512>>>(width, height, zBufferProperty, zBuffer, filterLUT, numSurfels, surfels);
