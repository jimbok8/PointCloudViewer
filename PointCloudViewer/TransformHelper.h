...
static Eigen::Matrix4f lookAt(const Eigen::Vector3f& posotion, const Eigen::Vector3f& center, const Eigen::Vector3f& up) {
    Eigen::Vector3f f = (center - posotion).normalized();
    ...
    ans(0, 1) = s(1);
    ans(0, 2) = s(2);
    ans(0, 3) = -s.dot(posotion);

    ans(1, 0) = u(0);
    ans(1, 1) = u(1);
    ans(1, 2) = u(2);
    ans(1, 3) = -u.dot(posotion);

    ans(2, 0) = -f(0);
    ans(2, 1) = -f(1);
    ans(2, 2) = -f(2);
    ans(2, 3) = f.dot(posotion);
...
static Eigen::Matrix4f perspective(const float fovy, const float aspect, const float zNear, const float zFar) {
