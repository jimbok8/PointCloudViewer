...
private:
    ...
    float calculateRadius(const int p0, const int p1, const int p2) const;
    Eigen::Vector3f calculateNormal(const int p0, const int p1, const int p2) const;
    CCandidate calculateCandidate(const std::vector<Eigen::Vector3f>& points, const std::vector<float>& radii, const std::vector<bool>& flag, const std::vector<std::pair<int, int>>& candidates, const int source, const int target, const Eigen::Vector3f& normal, const float maximumRadius) const;
    void addEdge(const std::vector<Eigen::Vector3f>& points, std::map<std::pair<int, int>, Eigen::Vector3f>& edges, std::priority_queue<CCandidate>& heap, const std::vector<float>& radii, const std::vector<bool>& flag, const std::vector<std::pair<int, int>>& candidates, const int source, const int target, const Eigen::Vector3f& normal, const float maximumRadius) const;
