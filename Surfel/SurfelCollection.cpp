...
void SurfelCollection::setPropertyDescriptor (const SurfelInterface::PropertyDescriptor newPropertyDescriptor, bool emitSignal) {
	...
	printf("Converting surfel properties...\n", -1);
	...
	if ((newPropertyDescriptor & SurfelInterface::DIFFUSE_COLOR) && !diffuseColors) {
		diffuseColors = new std::vector<COLORREF>(nCapacity);
		for (i = 0; i < nOfSurfels; i++) {
	...
	if ((newPropertyDescriptor & SurfelInterface::SPECULAR_COLOR) && !specularColors) {
		specularColors = new std::vector<COLORREF>(nCapacity);
		for (i = 0; i < nOfSurfels; i++) {
	...
	printf("Surfel properties converted.\n");
}
...
SurfelInterface *SurfelCollection::addSurfel (const Vector3D position, const Vector3D normal,
	...
	const float r, const COLORREF diffuseColor, const COLORREF specularColor, 
	...
	const bool emitSignal) {
		...
		if (nOfSurfels >= nCapacity) {
			reserve(nOfSurfels + 1 + (int)(nCapacity*incrCapacityFactor));
		}
...
SurfelInterface *SurfelCollection::addSurfel (const bool emitSignal) {
	...
	nOfSurfels++;
	if (nOfSurfels > nCapacity) {
		reserve(nOfSurfels + (int)(nCapacity*incrCapacityFactor));
	}
...
void SurfelCollection::removeSurfels (const std::list<SurfelInterface> *removeSurfels, const bool emitSignal) 
{
	std::list<SurfelInterface>::const_iterator it = removeSurfels->begin();
	for ( ; it != removeSurfels->end(); ++it) {
		removeSurfel(it->index);
	}
...
COLORREF SurfelCollection::getDiffuseColorNative(const int index) {
	return AT(diffuseColors,index);
}

COLORREF SurfelCollection::getSpecularColorNative(const int index) {
...
void SurfelCollection::setDiffuseColor(const int index, const COLORREF newDiffuseColor) {
	AT(diffuseColors,index) = newDiffuseColor;
}

void SurfelCollection::setSpecularColor(const int index, const COLORREF newSpecularColor) {
...
void SurfelCollection::combineColorProperties(const int index, const SurfelInterface *copySurfel1, const SurfelInterface *copySurfel2, const float factor) {
	...
	if (ambientCoefficients) {
		AT(ambientCoefficients,index) = factor*copySurfel1->getAmbientCoefficient() + (1.0f - factor)*copySurfel2->getAmbientCoefficient();
	}
	if (diffuseCoefficients) {
		AT(diffuseCoefficients,index) = factor*copySurfel1->getDiffuseCoefficient() + (1.0f - factor)*copySurfel2->getDiffuseCoefficient();
	}
	if (specularCoefficients) {
		AT(specularCoefficients,index) = factor*copySurfel1->getSpecularCoefficient() + (1.0f - factor)*copySurfel2->getSpecularCoefficient();
	}
	if (shininess) {
		AT(shininess,index) = factor*copySurfel1->getShininess() + (1.0f - factor)*copySurfel2->getShininess();
	}
}
...
COLORREF SurfelCollection::getDiffuseColorEmpty(const int index) {
	//qDebug("Function getDiffuseColor is not implemented");
	// return dummy value
	return RGB(240, 240, 240);
}

COLORREF SurfelCollection::getSpecularColorEmpty(const int index) {
...
void SurfelCollection::setDiffuseColorEmpty(const int index, const COLORREF newDiffuseColor) {
...
void SurfelCollection::setSpecularColorEmpty(const int index, const COLORREF newSpecularColor) {
