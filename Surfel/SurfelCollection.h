...
public:
	 ...
	 */
	static const MyDataTypes::TextureCoordinate defaultTextureCoordinate;
	 ...
	 *		
	 */
	SurfelCollection ( const SurfelInterface::PropertyDescriptor propertyDescriptor = SurfelInterface::BaseSurfel_PropertyDescriptor,
	 ...
	 */
	void setPropertyDescriptor (const SurfelInterface::PropertyDescriptor newPropertyDescriptor, bool emitSignal = true);
	 ...
	 * @see #getComputedPropertyDescriptor
	 */
	SurfelInterface::PropertyDescriptor getPropertyDescriptor() const;
	...
	* @see #getPropertyDescriptor
	*/
	SurfelInterface::PropertyDescriptor getComputedPropertyDescriptor() const;
	 ...
	 * @see #resize
	 */
	inline unsigned int getNofSurfels() const;
	 ...
	 */
	SurfelInterface *getFirstSurfel();
	 ...
	 * @see #getPrevSurfel
	 */
	SurfelInterface *getCurrentSurfel() const;
	 ...
	 */
	SurfelInterface *getSurfel(const int index);


    /**
     * returns the surfel vector
	 */
	std::vector<SurfelInterface*>* getSurfelVector();

    /**
	 * returns the position vector
	 */
	std::vector<Vector3D>* getPositionVector();

	/**
	 * returns the normal vector
	 */
	std::vector<Vector3D>* getNormalVector();
	 ...
	 * @see #getNofSurfels()
	 */
	unsigned int size() const { return getNofSurfels(); }
	 ...
	 */
	bool contains (const SurfelInterface *surfel);
	 ...
	 * @param diffuseColor
	 *        the <code>COLORREF</code> value of the diffuse color of the surfel
	 * @param specularColor
	 *        the <code>COLORREF</code> value of the specular color of the surfel
	 * @param flags
	 ...
	 */
	SurfelInterface *addSurfel (const Vector3D position, const Vector3D normal,
		                                 const Vector3D axisOne, const Vector3D axisTwo,
		                                 const float radius, const COLORREF diffuseColor, const COLORREF specularColor, 
	 ...
	 * @see #surfelCollectionChanged
	 */
	SurfelInterface *addSurfel (const bool emitSignal = true);
	 ...
	 * @see #surfelCollectionChanged
	 */
	SurfelInterface *addSurfel (const SurfelInterface *surfel, const bool emitSignal = true);
	 ...
	 * @see #surfelCollectionChanged
	 */
	void addSurfels (std::vector<SurfelInterface*> *surfels, const bool emitSignal = true);
	 ...
	 * @see #surfelCollectionChanged
	 */
	bool removeSurfel (SurfelInterface *surfel, const bool emitSignal = true);
	 ...
	 * @see #surfelCollectionChanged
	 */
	void removeSurfels (const std::list<SurfelInterface> *surfels, const bool emitSignal = true);
	 ...
	 * @see #surfelCollectionChanged
	 */
	void removeSurfels (const std::vector<SurfelInterface *>* surfels, const bool emitSignal = true);
	 ...
	 *        re-rendered; use with <b>caution</b>
	 */
	void copy (SurfelCollection *sourceCollection, const bool emitSignal = true);
	 ...
	 * @see #surfelCollectionChanged
	 */
	void clear (const bool emitSignal = true);
	 ...
	 *        re-rendered; use with <b>caution</b>
	 */
	void resize(const int newNOfSurfels, const bool emitSignal = true);
	 ...
	 * @see #capacity
	 */
	void setIncrementCapacityFactor(const float incrCapacityFactor);
...
private:
	...
	COLORREF						(SurfelCollection::*_getDiffuseColor)(const int index) ;
	COLORREF						(SurfelCollection::*_getSpecularColor)(const int index) ;
	...
	void							(SurfelCollection::*_setDiffuseColor)(const int index, const COLORREF newDiffuseColor);
	void							(SurfelCollection::*_setSpecularColor)(const int index, const COLORREF newSpecularColor);
...
private:
	...
	COLORREF							getDiffuseColorNative(const int index);
	COLORREF							getSpecularColorNative(const int index);
	...
	void							setDiffuseColor(const int index, const COLORREF newDiffuseColor);
	void							setSpecularColor(const int index, const COLORREF newSpecularColor);
	...
	COLORREF							getDiffuseColorEmpty(const int index);
	COLORREF							getSpecularColorEmpty(const int index);
	...
	void							setDiffuseColorEmpty(const int index, const COLORREF newDiffuseColor);
	void							setSpecularColorEmpty(const int index, const COLORREF newSpecularColor);
...
protected:
	...
	std::vector<COLORREF>							*diffuseColors;
	std::vector<COLORREF>							*specularColors;
...
private:
	...
	inline SurfelInterface* createNewSurfel(const int index);
	// removes and destroies a surfel
	inline void removeSurfel(const int index);
	// returns fac*color1 + (1.0-fac)*color2
	inline COLORREF mixColor(const COLORREF color1, const COLORREF color2, const float fac) const;
};
...
inline unsigned int SurfelCollection::getNofSurfels() const {
	return nOfSurfels;
}

inline COLORREF SurfelCollection::mixColor(const COLORREF color1, const COLORREF color2, const float fac) const
{

	return RGB(  (int)((float)GetRValue(color1)*fac + (float)GetRValue(color2)*(1.0-fac)+0.5f),
				(int)((float)GetGValue(color1)*fac + (float)GetGValue(color2)*(1.0-fac)+0.5f),
				(int)((float)GetBValue(color1)*fac + (float)GetBValue(color2)*(1.0-fac)+0.5f)
				 );
}
