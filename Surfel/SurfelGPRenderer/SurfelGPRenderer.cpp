...
void SurfelGPRenderer::initialize (const bool isVisible/*, QWidget *parent*/) {
	...
	MtrUtil::MtrUnity4x4f (sceneViewMatrix);
	...
	iWidth  = viewPortSize.cx  / superSampling;
	iHeight = viewPortSize.cy / superSampling;	
...
CImage* SurfelGPRenderer::getRenderedImage() {
...
void SurfelGPRenderer::setViewFrustum (const MyDataTypes::ViewFrustum newViewFrustum) {
	...
	printf ("SurfelGPRenderer: setViewFrustum: fov: %f, aspect: %f\n", newViewFrustum.fieldOfView, newViewFrustum.aspectRatio);
	frameBuffer->setValid (false);
...
void SurfelGPRenderer::setViewPortSize (const CSize newViewPortSize) {
	...
	iWidth  = frameBuffer->getSize().cx  / superSampling;
	iHeight = frameBuffer->getSize().cy / superSampling;
...
CSize SurfelGPRenderer::getViewPortSize() const {
...
void SurfelGPRenderer::setBackgroundColor(COLORREF newBackgroundColor) {
	backgroundColor = newBackgroundColor;
}

COLORREF SurfelGPRenderer::getBackgroundColor() const {
...
void SurfelGPRenderer::renderFrame (const int attributes, const bool allScene) 
{
	...
	SrfSetProgressiveSize (surfelContext, frameBuffer->getSize().cx, frameBuffer->getSize().cy);
	...
	std::list<Object*> &object_list = scene->getObjectList();
	std::list<Object*>::iterator itr = object_list.begin();
	while(itr!=object_list.end())
	{
		object = *itr;
		if (object->isFlagOn (Object::VISIBLE)) {
			if (allScene || object == scene->getActiveObject()) {
				...
				MtrUtil::MtrMult4x4f (sceneViewMatrix, objectTransformation, finalTransformation);

				// make it a row major matrix
				MtrUtil::MtrTranspose4x4f (finalTransformation, convertedTransformation);
...
bool SurfelGPRenderer::clipLine(Vector2D start, Vector2D end, int *Ax, int *Ay, int *Bx, int *By) {
	CSize viewPortSize;
	//CSize viewPortSize = Configuration::getInstance()->getViewPortSize();
	bool accept = false;
	bool done = false;
	float x,y;
	...
	float xmin = 0;
	float ymin = 0;
	float xmax = (float)viewPortSize.cx-1;
	float ymax = (float)viewPortSize.cy-1;
...
void SurfelGPRenderer::drawLine (const Vector3D start, const Vector3D end, COLORREF color) {
	float depth;
	int Ax, Ay, Bx, By;
...
void SurfelGPRenderer::renderShadowBuffer () {
	...
	MtrUtil::MtrCopy4x4f (sceneViewMatrix, restoreSceneViewMatrix);

	MtrUtil::MtrUnity4x4f (lightViewMatrix);
