...
static const unsigned int NOF_SEQ_DIGITS = 5;
...
unsigned int Scene::getNofObjects() const {
	return objects.size();
}

unsigned int Scene::getNofEnabledObjects() 
{
	unsigned int   nofEnabledObjects = 0;
	std::list<Object*>::iterator itr = objects.begin();
	while(itr != objects.end())
	{
		if ((*itr)->isFlagOn (Object::ENABLED)) 
		{
			nofEnabledObjects++;
		}
		itr++;
...
unsigned int Scene::getNofSurfels() 
{
	unsigned int   nofSurfels;
	nofSurfels = 0;
	std::list<Object*>::iterator itr = objects.begin();
	while(itr != objects.end())
	{
		nofSurfels += (*itr)->getSurfelCollection()->getNofSurfels();
		itr++;
	}
	return nofSurfels;
...
std::string Scene::getUniqueName (const std::string &name) 
{
	bool    found;
	std::string objectName, uniqueName;
	unsigned int    size;
	int	    sequenceNumber, highestSequenceNumber;
	...
	while(itr != objects.end())
	{
		objectName = (*itr)->getName();
		//if (strncmp (name.latin1(), objectName.latin1(), size) == 0) 
		if (strncmp (name.c_str(), objectName.c_str(), size) == 0) 
		{
...
std::string Scene::addObject (Object *newObject, const bool makeActive, const bool emitSignal) {

	std::string uniqueName;

	uniqueName = this->getUniqueName (newObject->getName());
	newObject->setName (uniqueName.c_str(), false);
	...
	if (objects.size() > 0) {
		if (makeActive == true) {
			...
			activeObjectID = (int)objects.size()-1;
...
bool Scene::removeObject (Object *object, const bool emitSignal) {
	...
	bool successFlag = (it != objects.end());
...
void Scene::setRotation (const float angle, const float x, const float y, const float z, const bool emitSignal) {
	...
	MtrUtil::MtrCreateRotation4x4fc (angle, x, y, z, cameraPosition.rotationMatrix);
...
void Scene::rotate (const float dAngle, const float x, const float y, const float z, const bool emitSignal) {
	...
	MtrUtil::MtrMult4x4f (userRotation, cameraPosition.rotationMatrix, finalRotation);
	MtrUtil::MtrCopy4x4f (finalRotation, cameraPosition.rotationMatrix);
...
void Scene::getRotationMatrix (MyDataTypes::TransformationMatrix16f rotationMatrix) const {
	MtrUtil::MtrCopy4x4f (cameraPosition.rotationMatrix, rotationMatrix);
}
...
void Scene::setSurfelDiffuseColor (SurfelInterface *surfel, const COLORREF newDiffuseColor, const bool emitSignal) {
...
void Scene::setSurfelSpecularColor (SurfelInterface *surfel, const COLORREF newSpecularColor, const bool emitSignal) {
...
void Scene::removeSurfels (Object *object, std::list<SurfelInterface> *surfels, const bool emitSignal) {
...
void Scene::updateTransformationMatrix() {
	MtrUtil::MtrMult4x4f (cameraPosition.scaleTranslationMatrix, cameraPosition.rotationMatrix, transformationMatrix);
